export default class Board  {
     ...
     * @returns {boolean}
     */
    constructor(width: number, height: number, mineNbr: number,menu: any) {
    ...
    setNumber(): void{
        for (let cell_ID = 0; cell_ID < this.cellList.length; cell_ID++) {
            ...
            if(this.cellList[cell_ID].dataset.value !== 'M') {
                ...
                // TOP LEFT
                if( x>0 && y>0){
                    if(this.cellList[x-1 + ((y-1) * this.width)].dataset.value == 'M') nbrOfMines++;
                }

                // MIDDLE LEFT
                if( x> 0) {
                    if(this.cellList[x-1 + ((y) * this.width)].dataset.value == 'M') nbrOfMines++;
                }
								...
								// BOTOM MIDDLE
                if(y<this.height-1){
                    if(this.cellList[x + ((y+1) * this.width)].dataset.value == 'M') nbrOfMines++;
                }
    ...
    flagged(cell_ID: number):void{
        if(this.cellList[cell_ID].classList.contains('flagged')){
            this.cellList[cell_ID].classList.remove('flagged');
            this.flagSet --;
            if(this.cellList[cell_ID].dataset.value === 'M') this.trueFlag --;
        } else {
        ...
        this.updateAffichage((this.mineNbr - this.flagSet).toString());
        if(this.trueFlag === this.mineNbr) this.win();
    }

    updateAffichage(str :string): void{
